...
%             xticks([0:45:360] + crank_angles(1))
            ...
            gax2 = gca;
            gax2.FontSize = fs;
            trial_name = ['S_{x,y}=(',num2str(Sx(i)),',',num2str(Sy(j)),')'];
...
tile_1.InnerPosition = [0.07    0.10    0.725    0.85];
...
mean_metabolics = zeros([length(Sx), length(Sy)]);
colororder([0.8 0 0; 0 0 0.8; 0 0.8 0; 1 0 0.8; 
                0 0.7 0.8; 1 0.5 0; 0.5 0 1; 0 0.5 0])
            
fig6 = figure(6); clf;
fig6.Name = 'Metabolics w.r.t. Crank Angle';
tile_6 = tiledlayout(3,3,'Padding','compact');
...
for j = 1:length(Sy)
    for i = 1:length(Sx)
        ...
        if Sx(i)==-0.11 && (Sy(j)== -0.04)
            ...
            gax6.FontSize = fs;   
            for mi = meta_index
                smooth_metabolics = smoothdata(metabolics(:,mi),'movmean',30);
...
tile_6.InnerPosition = [0.065    0.10    0.70    0.85];
