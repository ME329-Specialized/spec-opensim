...
if startsWith(pwd, 'C:\Users\david')
    cd('C:\Users\david\GitHub\spec-opensim\full_workflow_02272021_03082021_Caittesting_FORDAVID\full_workflow');
end
...
for j = 1:length(Sy)
    for i = 1:length(Sx)
        ...
        if Sx(i)==-0.11 && (Sy(j)== -0.04)
            ...
            [t, ham_lh, ham_sh, glut, vast, P] = get_mean_total_metabolic_rate(Sx(i), Sy(j), k, plotting);
            time{k} = t;
...
parachute = figure(k+1); 
clf; box on; grid on; hold on;
set(gcf,'Name','Sum Total Metabolics')
parachute.OuterPosition(2) = 150;
parachute.OuterPosition(3) = 1100;
paraachute.OuterPosition(4) = 700;
set(gca,'FontSize',fs)
...
set(gca,'FontSize',fs)
vast_cmp.OuterPosition(2) = 150;
vast_cmp.OuterPosition(3) = 1100;
vast_cmp.OuterPosition(4) = 700;
set(groot,'DefaultLineLineWidth', 1.5);
plot(time{1}, vas_int{1}, 'DisplayName', 'S_{x,y} = (-0.10,0.04) m')
plot(time{2}, vas_int{2}, 'DisplayName', 'S_{x,y} = (-0.09,0.04) m')
plot(time{3}, vas_int{3}, 'DisplayName', 'S_{x,y} = (-0.11,0.05) m')
...
xlabel('Time [s]')
xticks(0:0.1:1)
lgd = legend('FontSize',fs,'AutoUpdate','off');
lgd.Position = [0.6 0.25 0.25 0.5];
axis([0 0.75 0 inf])
hold off;